...
const goBack = () => {
  ...
  else router.visit(route('bibliotecas.index'))
}
</script>

<template>
    <Head title="Crear biblioteca" />

    <AuthenticatedLayout>
        <form
            @submit.prevent="form.post(route('bibliotecas.store'))"
        ...
        >
            <div class="flex flex-col md:flex-row gap-4 mb-6">
                <div class="w-full md:w-1/2">
                    ...
                    <TextInput
                        id="nombre"
                        type="text"
                        class="mt-1 block w-full rounded-full h-11 px-4 text-sm focus:border-brandgold focus:ring-brandgold dark:border-white dark:bg-gray-800 dark:text-slate-100"
                        v-model="form.nombre"
                        required
                        maxlength="100"
            ...
            <div class="flex justify-center gap-6 mt-8">
                <button
                    type="button"
                    @click="goBack"
                    class="px-6 py-2 w-full mx-20 rounded-full border-2 border-brandblue text-brandblue font-bold hover:bg-brandblue hover:text-white transition dark:border-white dark:bg-gray-100 dark:text-gray-800 dark:hover:bg-gray-300"
                >
                    CANCELAR
                </button>
                <button
